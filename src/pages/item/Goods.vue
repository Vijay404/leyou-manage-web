    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-else @click="editSeckill(props.item)">秒杀</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      editSaleable(oldGoods){
        // 弹窗确认是否上架/下架
        this.$message.confirm("确认要"+(oldGoods.saleable === true ? "下架" : "上架")+"商品吗？")
          .then(() => {
            // 发起异步请求修改商品的saleable属性值
            this.$http.put("/item/goods/saleable?id="+oldGoods.id+"&saleable="+oldGoods.saleable);
          }).then(() => {
          // 修改成功重新加载数据
